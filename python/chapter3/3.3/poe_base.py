...
def vec_to_se3(S):
    """Convert a 6D spatial velocity vector into an se(3) matrix. =>[S]"""
    w = S[:3]
    v = S[3:]
    se3 = np.zeros((4, 4))
    se3[:3, :3] = skew(w)
...
def fk_poe(S_list, M, theta_list):
    """
    Compute forward kinematics using the Product of Exponentials formula.

    Parameters:
    - S_list: list of 6D screw axes in the base frame (shape: [n, 6])
    ...
    for S, theta in zip(S_list, theta_list):
        exp_se3 = matrix_exp6(vec_to_se3(S) * theta) # e^[s]*theta
        T = T @ exp_se3 # post multiplication with e^[s]*theta
    T = T @ M
    return T
